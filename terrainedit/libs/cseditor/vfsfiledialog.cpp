...
BEGIN_EVENT_TABLE (VFSFileDialog, wxDialog)
  ...
  EVT_TREE_ITEM_EXPANDING(DIRECTORY_TREE, VFSFileDialog::OnDirTreeExpandBranch)
  EVT_TREE_ITEM_COLLAPSING(DIRECTORY_TREE, VFSFileDialog::OnDirTreeCollapseBranch)
  EVT_TREE_ITEM_RIGHT_CLICK(DIRECTORY_TREE, VFSFileDialog::OnRightClickItem)

  EVT_LIST_ITEM_SELECTED(DIRECTORY_LIST, VFSFileDialog::OnDirViewSelChange )
  EVT_LIST_ITEM_ACTIVATED(DIRECTORY_LIST, VFSFileDialog::OnDirViewActivated )

  EVT_TEXT_ENTER( TEXT_PATH, VFSFileDialog::OnPathTextEnter )

  EVT_BUTTON( BUTTON_CANCEL, VFSFileDialog::OnCancel )
  EVT_BUTTON( BUTTON_OPEN, VFSFileDialog::OnOk )
  EVT_BUTTON( BUTTON_PARENT, VFSFileDialog::OnOpenParent)

  EVT_CHOICE(CHOICE_VIEW_MODE, VFSFileDialog::OnViewModeSelect ) 
END_EVENT_TABLE()

VFSFileDialog::VFSFileDialog ( wxWindow *parent,
			       wxWindowID id,
...
VFSFileDialog::~VFSFileDialog()
{
}

//Operations
wxImageList* VFSFileDialog::CreateImageList()
{
...
csString VFSFileDialog::GetPathByTreeItem(wxTreeItemId id)
{
  csString path = "";
  wxTreeItemId cur = id;
...
bool VFSFileDialog::HasChildDir( wxTreeItemId parent )
{
...
void VFSFileDialog::InitViewTypeList()
{
...
bool VFSFileDialog::IsDir( const char* path )
{
  char lastChar = 0;
...
bool VFSFileDialog::IsFile( const char* path )
{
  char lastChar = 0;
  lastChar = path[ strlen( path ) -1 ];
...
bool VFSFileDialog::LoadVFSDirView(const char *path)
{
...
bool VFSFileDialog::LoadVFSTreeBranch(wxTreeItemId parent, const char *path)
{
  csRef<iStringArray> childList = vfs->FindFiles(path);
...
void VFSFileDialog::OpenTreeItemChild( wxString childName )
{
...
void VFSFileDialog::OnCancel( wxCommandEvent& event )
{
  //Return FALSE tells main frame to do nothing
  EndModal(FALSE);
}

void VFSFileDialog::OnDirTreeCollapseBranch( wxTreeEvent& event )
{
  //wxTreeItemId curSelection = event.GetItem();
  //dirtree->SetItemImage( curSelection, imageIdList[IMAGE_FOLDER_CLOSED] );
}

void VFSFileDialog::OnDirTreeExpandBranch( wxTreeEvent& event )
{
  wxTreeItemId curSelection = event.GetItem();
...
void VFSFileDialog::OnDirTreeSelChange(wxTreeEvent& event)
{
...
void VFSFileDialog::OnDirViewActivated( wxListEvent & event )
{
...
void VFSFileDialog::OnDirViewSelChange(wxListEvent& event)
{
...
void VFSFileDialog::OnOk( wxCommandEvent& event )
{
...
void VFSFileDialog::OnPathTextEnter( wxCommandEvent& event )
{
  if( IsFile( filename ) )
  {
    EndModal(TRUE);
...
void VFSFileDialog::OnOpenParent( wxCommandEvent& event )
{
...
void VFSFileDialog::OnViewModeSelect( wxCommandEvent& event )
{
...
void VFSFileDialog::OnRightClickItem( wxTreeEvent& event)
{
  /// do nothing
}
